...
window.onload = function () {
    var chart = new OrgChart(document.getElementById("tree"), {
        ...
        nodes: [
            ...
            , link: "<?= base_url(); ?>home/genealogy_tree/tree/<?= $sponsor['sponsor_id']; ?>"},

             <?php foreach ($memberlist as $level_one): ?>

                { id: "<?= $level_one['member_id']; ?>", pid: "<?= $level_one['parent_id']; ?>", name: "<?= $level_one['profile_id']; ?>", title: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $level_one['sponsor_id'], 'username'); ?> [<?= $this->db->get_where('sponsor_tree' , array('sponsor_tree_id >' => $level_one['sponsor_tree_id']))->num_rows() ?>]", email: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $level_one['sponsor_id'], 'email'); ?>", img: "<?= base_url(); ?>assets/site_img/network.jpg" , link:"<?= base_url(); ?>home/genealogy_tree/tree/<?= $level_one['sponsor_id']; ?>" },

                  <?php foreach ($level_one['L_1'] as $two): ?>
                    { id: "<?= $two['member_id']; ?>", pid: "<?= $two['parent_id']; ?>", name: "<?= $two['profile_id']; ?>", title: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $two['sponsor_id'], 'username'); ?> [<?= $this->db->get_where('sponsor_tree' , array('sponsor_tree_id >' => $two['sponsor_tree_id']))->num_rows() ?>]", email: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $two['sponsor_id'], 'email'); ?>", img: "<?= base_url(); ?>assets/site_img/network.jpg", link:"<?= base_url(); ?>home/genealogy_tree/tree/<?= $two['sponsor_id']; ?>"},

                       <?php foreach ($two['L_2'] as $three): ?>
                         { id: "<?= $three['member_id']; ?>", pid: "<?= $three['parent_id']; ?>", name: "<?= $three['profile_id']; ?>", title: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $three['sponsor_id'], 'username'); ?> [<?= $this->db->get_where('sponsor_tree' , array('sponsor_tree_id >' => $three['sponsor_tree_id']))->num_rows() ?>]", email: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $three['sponsor_id'], 'email'); ?>", img: "<?= base_url(); ?>assets/site_img/network.jpg", link:"<?= base_url(); ?>home/genealogy_tree/tree/<?= $three['sponsor_id']; ?>"},
                         
                          <?php foreach ($three['L_3'] as $four): ?>
                              { id: "<?= $four['member_id']; ?>", pid: "<?= $four['parent_id']; ?>", name: "<?= $four['profile_id']; ?>", title: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $four['sponsor_id'], 'username'); ?> [<?= $this->db->get_where('sponsor_tree' , array('sponsor_tree_id >' => $four['sponsor_tree_id']))->num_rows() ?>]", email: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $four['sponsor_id'], 'email'); ?>", img: "<?= base_url(); ?>assets/site_img/network.jpg", link:"<?= base_url(); ?>home/genealogy_tree/tree/<?= $four['sponsor_id']; ?>"},
                                
                                <?php foreach ($four['L_4'] as $five): ?>
                                    { id: "<?= $five['member_id']; ?>", pid: "<?= $five['parent_id']; ?>", name: "<?= $five['profile_id']; ?>", title: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $five['sponsor_id'], 'username'); ?> [<?= $this->db->get_where('sponsor_tree' , array('sponsor_tree_id >' => $five['sponsor_tree_id']))->num_rows() ?>]", email: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $five['sponsor_id'], 'email'); ?>", img: "<?= base_url(); ?>assets/site_img/network.jpg", link:"<?= base_url(); ?>home/genealogy_tree/tree/<?= $five['sponsor_id']; ?>"},
                                   
                                   <?php foreach ($five['L_5'] as $six): ?>
                                         { id: "<?= $six['member_id']; ?>", pid: "<?= $six['parent_id']; ?>", name: "<?= $six['profile_id']; ?>", title: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $six['sponsor_id'], 'username'); ?> [<?= $this->db->get_where('sponsor_tree' , array('sponsor_tree_id >' => $six['sponsor_tree_id']))->num_rows() ?>]", email: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $six['sponsor_id'], 'email'); ?>", img: "<?= base_url(); ?>assets/site_img/network.jpg", link:"<?= base_url(); ?>home/genealogy_tree/tree/<?= $six['sponsor_id']; ?>"},
                                         
                                         <?php foreach ($six['L_6'] as $seven): ?>
                                             { id: "<?= $seven['member_id']; ?>", pid: "<?= $seven['parent_id']; ?>", name: "<?= $seven['profile_id']; ?>", title: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $seven['sponsor_id'], 'username'); ?> [<?= $this->db->get_where('sponsor_tree' , array('sponsor_tree_id >' => $seven['sponsor_tree_id']))->num_rows() ?>]", email: "<?= $this->Crud_model->get_type_name_by_id('sponsor', $seven['sponsor_id'], 'email'); ?>", img: "<?= base_url(); ?>assets/site_img/network.jpg", link:"<?= base_url(); ?>home/genealogy_tree/tree/<?= $seven['sponsor_id']; ?>"},
                                         <?php endforeach ?>
